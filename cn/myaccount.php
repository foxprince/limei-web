...
if (isset ( $_GET ['delete'] )) {
	...
	$stmt_user_update = $conn->query ( $sql_delete );
}
...
if ($historyfound) {
	...
	foreach ( $stmt_history as $row_history ) {
		...
		$demopiclink = '/cn/img-eles/goodprice.png';
...
<h3>钻石参数：</h3>
            <p class="dia-para">
              <label class="dia-label">克拉:</label><?php echo $user_diamond_carat; ?>克拉</p>
            <p class="dia-para">
              <label class="dia-label">颜色:</label><?php echo $user_diamond_color; ?></p>
            <p class="dia-para">
              <label class="dia-label">净度:</label><?php echo $user_diamond_clarity; ?></p>
            <p class="dia-para">
              <label class="dia-label">切工:</label>切割：<?php echo $user_diamond_cut; ?> | 抛光：<?php echo $user_diamond_polish; ?> | 对称性：<?php echo $user_diamond_symmetry; ?></p>
            <p class="dia-para">
              <label class="dia-label">荧光:</label><?php  echo $user_diamond_fluo; ?></p>
            <p class="dia-para">
              <label class="dia-label">出品地:</label>比利时 安特卫普
            </p>
            <p class="dia-para">
              <label class="dia-label">证书编号:</label><?php echo $user_diamond_certificate_number; ?></p>
            <p class="dia-para">
              <label class="dia-label">价格:</label><?php echo $user_diamond_price; ?> 美元</p>
            <p class="dia-para">
              <label class="dia-label">查看证书:</label>
            
            
            <p class="dia-para">
              <a class="button" href="myaccount.php?delete=<?php echo $row_history['id'];?>">删除</a>
            </p>
<?php
		if ($user_diamond_grading_lab == 'HRD') {
			?>
<a class="certi_linker" target="_blank"
              href="http://www.hrdantwerplink.be/index.php?record_number=<?php echo $user_diamond_certificate_number; ?>&weight=<?php echo $user_diamond_carat; ?>&L="><img
              id="gradinglabicon" src="../images/site_elements/<?php echo strtoupper($user_diamond_grading_lab); ?>.png" /></a>
<?php
		} else if ($user_diamond_grading_lab == 'GIA') {
			?>
<a class="certi_linker" target="_blank"
              href="http://www.gia.edu/cs/Satellite?pagename=GST%2FDispatcher&childpagename=GIA%2FPage%2FReportCheck&c=Page&cid=1355954554547&reportno=<?php echo $user_diamond_certificate_number; ?>"><img
              id="gradinglabicon" src="../images/site_elements/<?php echo strtoupper($user_diamond_grading_lab); ?>.png" /></a>
<?php
		} else if ($user_diamond_grading_lab == 'IGI') {
			?>
<a class="certi_linker" target="_blank"
              href="http://www.igiworldwide.com/igi/verify.php?r=<?php echo $user_diamond_certificate_number; ?>"><img
              id="gradinglabicon" src="../images/site_elements/<?php echo strtoupper($user_diamond_grading_lab); ?>.png" /></a>
<?php
		}
		?>
