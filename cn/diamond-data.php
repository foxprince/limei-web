...
//}
$query_weight = '(carat >= '.$query_weight_from.' AND carat <= '.$query_weight_to.')';
...
$sql_count='SELECT COUNT(*) AS num FROM diamonds WHERE visiable=1 AND '.$query_shape.$query_color.$query_clarity.$query_cut.$query_polish.$query_sym.$query_fluo.$query_certi.$and.$query_weight.' AND (price BETWEEN '.$query_price_from.' AND '.$query_price_to.') AND status = "AVAILABLE" '.$featured;
foreach($conn->query($sql_count) as $num){
	$result_number=$num['num'];
}
/**/

$sql='SELECT * FROM diamonds WHERE visiable=1 AND '.$query_shape.$query_color.$query_clarity.$query_cut.$query_polish.$query_sym.$query_fluo.$query_certi.$and.$query_weight.' AND (price BETWEEN '.$query_price_from.' AND '.$query_price_to.') AND status = "AVAILABLE" '.$featured.' '.$query_sorting.' LIMIT '.$startfrom.', 35';
logger($sql);
//exit($sql);
