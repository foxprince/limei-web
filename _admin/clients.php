		...
		$.post("createQrcode.php", 
			...
			function(data){
				if($.trim(data)=='OK'){
					...
					$('td[id='+btnId+']').html('<a id="qr_image" href="./qrcode/'+btnId+'.jpg" target="_blank"><span style="color: red">查看</span></a>');
				}else{
...
<form action="./clients.php" id="addnew">
  <p>
  <label>微信名称:</label>
  <input name="wechatName" id="cate_cn" type="text" class="formbox" value="<?php echo $wechat_name?>"/>
  <label>二维码:</label>
 	<select name="isSetQrcode"><option value="2">全部</option><option value="1" <?php $isSetQrcode==1?"selected":""?>>已生成</option><option value="0" <?php $isSetQrcode==0?"selected":""?>>未生成</option></select>
  
  
    <input type="submit" id="submit_article" value="search"style="font-size:24px; font-weight:bold; padding:5px 20px;"/> 
  </p>
</form>
...
<td align="center" id="<?php echo $row['id'] ?>">
	...
	if($row['qrcode']) {
		...
		?>
		<a id="qr_image" href="./qrcode/<?php echo $row['qrcode'] ?>"  target="_blank">查看</a><br/>
		共有<a href="clients.php?referee=<?php echo $row['id'] ?>"><?php echo $num ?></a>被推荐者
